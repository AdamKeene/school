...
public class SinkFilter extends FilterFramework
{
	public void run()
    {
		...
		*************************************************************************************/
		...
		long currentTime = 0;
		double velocity = 0, altitude = 0, pressure = 0, temperature = 0;
		boolean hasTime = false;

		PrintWriter writer = null;
		try {
			writer = new PrintWriter("OutputA.csv");
			writer.println("Time,Velocity,Altitude,Pressure,Temperature");
		} catch (IOException e) {
		...
		System.out.println( "\n" + this.getName() + "::Sink writing to OutputA.csv ");
		...
		while (true)
		{
			try
			{
				...
				*****************************************************************************/
				...
				/****************************************************************************
				// Process the ID and measurement
				****************************************************************************/
				if ( id == 0 )
				{
					if (hasTime) {
			...
			********************************************************************************/
			catch (EndOfStreamException e)
			{
				...
				writer.close();
				ClosePorts();
				System.out.println( "\n" + this.getName() + "::Sink done; bytes read: " + bytesread );
				break;
			}
		} // while
   } // run
